...
int gas_sensor=A1; //analog sensor for the gas-sensor input
int reading=0;
int buzzer=10;
int value_ppm=0;
...
void setup() {
  ...
  pinMode(gas_sensor,INPUT);//input from gas sensor
...
void loop() {
  ...
  reading=analogRead(gas_sensor);//taking reading from gas sensor
  value_ppm=map(gas_sensor,0,820,0,2000);//conversion of analog voltage into ppm 
  if(value_ppm>10)//taking the gas sensors value
  {
  ...
  if (Serial.available()) {    
    ...
    digitalWrite(LEDpin, LOW);
        ...
        Serial.print("");
        Serial.println(value_ppm);//printing the gas sensor values with time stamp 
